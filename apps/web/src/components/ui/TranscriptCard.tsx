 ...
 * reading.
 */
export function TranscriptCard({ entry, languages, layout, onReplay }: TranscriptCardProps) {
  ...
  useEffect(() => {
    ...
    // Entrance only. Character-by-character reveal happens in <LiveTranscript>
    // while the segment is still arriving; by the time an entry reaches this
    // component the text has settled, and re-animating it on every re-render
    // would restart finished sentences.
    const ctx = gsap.context(() => enterCard(cardRef.current!), cardRef);
    return () => ctx.revert();
  ...
  // MyMemory is a translation *memory*: a low score means it fell back to a
  // fuzzy hit rather than a real translation, which is worth flagging rather
  // than presenting with the same confidence as an exact result.
  const looseMatch = entry.matchQuality !== undefined && entry.matchQuality < 0.7;
      ...
      <div className="card__face card__face--front">
        ...
        <footer className="card__actions" data-stagger>
          <button
            ...
            aria-label={`Say the ${targetLang?.labelEn ?? 'translated'} text out loud again`}
          >
          ...
          {looseMatch && (
            <span className="card__meta" title="MyMemory returned a loose match — this translation may be approximate">
              approximate
            </span>
          )}
        </footer>
