...
/**
 * Implements the plan document's hard-stop behaviour (p.2), written there for
 * Azure but applied to whichever providers are in use:
 *
 *   quota exceeded → STOP API requests → Do NOT retry
 *                  → Tell user service limit was reached
 *
 * Once a provider reports an auth failure or a quota stop, every subsequent
 * call to it is refused locally. We stop hammering a service that has already
 * told us to go away — with MyMemory's small daily allowance, a retry loop
 * against a refusal is how you lose translation for the rest of the day.
 */
