/**
 * Decides which recognised transcripts are worth translating.
 *
 * Plan doc, p.4: "Do not send every tiny partial transcript to Translation.
 * Translate meaningful finalized segments instead."
 *
 * This matters more, not less, now that recognition is local: Whisper will
 * happily invent a word from a silent room, and every transcript that reaches
 * this gate costs MyMemory characters out of a small daily allowance.
 */
...
export function isProcessableAudio(durationSeconds: number, byteLength: number): boolean {
  // 512 bytes of 16 kHz 16-bit PCM is ~16 ms — comfortably below any real
  // utterance, and below the WAV header of an empty file.
