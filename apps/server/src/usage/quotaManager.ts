...
const REJECT_MESSAGES: Record<QuotaRejectReason, string> = {
  ...
  global_translate_char_limit: "The app has used today's translation allowance. Please try again tomorrow.",
 ...
 *             ALLOWED?
 *
 * Requests are authorised *before* any upstream call. Speech recognition is
 * local now, so the cost being controlled is CPU and MyMemory's daily character
 * allowance rather than a cloud bill — but the ceilings still exist for the
 * same reason: nothing should run unbounded.
 */
export class QuotaManager {
   ...
   * translation request" (500 chars — MyMemory's own hard limit) and the
   * app-wide **daily** character ceiling that keeps us inside MyMemory's free
   * allowance.
   *
  ...
  checkTranslation(input: { sessionId: string; text: string; inFlight?: boolean }): QuotaDecision {
    ...
    // MyMemory's allowance resets daily, not monthly — so the ceiling that
    // matters is today's, and it is app-wide rather than per user.
    const globalDay = this.#store.get(UsageStore.globalDay(this.#clock.now()));
    if (globalDay.translatedChars + input.text.length > this.#config.globalDailyTranslatedChars) {
