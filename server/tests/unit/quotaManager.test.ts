...
describe('QuotaManager', () => {
  ...
  describe('global free-tier ceilings', () => {
    it('stops translating before the app-wide monthly character ceiling is breached', () => {
      const { session } = quota.startSession('user-1');
      // Ceiling is 5,000 chars/month.
    ...
    it('stops synthesising before the app-wide monthly TTS ceiling is breached', () => {
      const { session } = quota.startSession('user-1');
      for (let i = 0; i < 50; i += 1) quota.commitTts(session.id, 100);

      const decision = quota.checkTts({ sessionId: session.id, text: 'hello' });

      expect(decision.allowed).toBe(false);
      expect(decision.reason).toBe('global_tts_char_limit');
    });
