...
export const TEST_CONFIG: AppConfigResponse = {
  ...
  limits: {
    ...
    dailySeconds: 600,
    monthlySeconds: 9000,
    maxCharsPerTranslation: 800,
    maxAudioBytes: 2_000_000,
  },
  providers: {
    stt: { name: 'azure-stt', mode: 'mock' },
    translate: { name: 'google-translate', mode: 'mock' },
    tts: { name: 'google-tts', mode: 'mock' },
  },
...
export function createMockServer(options: MockServerOptions = {}): MockServer {
  ...
  const fetchImpl = vi.fn(async (url: string, init?: RequestInit) => {
    ...
    if (path.endsWith('/translate/speak')) {
      return respond({
        status: 200,
        body: { ok: true, audioBase64: 'QUJD', mimeType: 'audio/mpeg', cached: true },
      });
    }
